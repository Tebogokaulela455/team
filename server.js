...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    db.run('INSERT INTO users (name,email,password) VALUES (?,?,?)', [name, email, hash], function(err) {
      ...
      res.json({ message: 'Registered. Free trial active for 30 days.' });
    });
  } catch {
...
app.get('/api/dependants', (req, res) => {
  db.all('SELECT * FROM dependants', [], (err, rows) => {
    if (err) return res.status(500).json({ error: 'DB error' });
    res.json(rows);
  });
});
...
app.put('/api/dependants/:id', (req, res) => {
  const { memberId, name, dob } = req.body;
  db.run('UPDATE dependants SET memberId=?,name=?,dob=? WHERE id=?', [memberId, name, dob, req.params.id], err => {
    if (err) return res.status(500).json({ error: 'DB error' });
    res.sendStatus(200);
  });
});
app.delete('/api/dependants/:id', (req, res) => {
  db.run('DELETE FROM dependants WHERE id=?', [req.params.id], err => {
    if (err) return res.status(500).json({ error: 'DB error' });
    res.sendStatus(200);
  });
});
...
app.post('/api/policies', (req, res) => {
  ...
  db.run('INSERT INTO policies (memberId,planType,coverLevel,premium,startDate,status) VALUES (?,?,?,?,?,?)', [memberId, planType, coverLevel, premium, startDate, status], function(err) {
    if (err) return res.status(500).json({ error: 'DB error' });
    res.json({ id: this.lastID });
  });
});
app.put('/api/policies/:id', (req, res) => {
  ...
  db.run('UPDATE policies SET planType=?,coverLevel=?,premium=?,startDate=?,status=? WHERE id=?', [planType, coverLevel, premium, startDate, status, req.params.id], err => {
    if (err) return res.status(500).json({ error: 'DB error' });
    res.sendStatus(200);
  });
});
...
app.delete('/api/agents/:id', (req, res) => {
  db.run('DELETE FROM agents WHERE id=?', [req.params.id], err => {
    if (err) return res.status(500).json({ error: 'DB error' });
    res.sendStatus(200);
  });
});
